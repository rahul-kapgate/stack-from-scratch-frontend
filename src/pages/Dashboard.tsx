...
const upcomingInterviews = [
  { id: 1, role: "Frontend Developer",  company: "TechCorp",    date: "Mar 25, 2026", time: "10:00 AM", status: "Confirmed" },
  { id: 2, role: "Full Stack Engineer", company: "StartupXYZ",  date: "Mar 28, 2026", time: "2:30 PM",  status: "Pending"   },
]

const pastInterviews = [
  { id: 1, role: "React Developer",  company: "Acme Inc.", date: "Mar 10, 2026", score: 82, result: "Passed" },
  { id: 2, role: "Backend Engineer", company: "DevStudio", date: "Mar 5, 2026",  score: 65, result: "Failed" },
  { id: 3, role: "SDE Intern",       company: "CloudBase", date: "Feb 28, 2026", score: 91, result: "Passed" },
]
    ...
    <div className="max-w-5xl mx-auto px-6 py-8 pb-24">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          {userLoading
            ? <div className="h-7 w-36 bg-muted animate-pulse rounded-md mt-1" />
        ...
        {tabs.map(t => (
          <button key={t} onClick={() => setActiveTab(t)}
            className={`px-4 py-2.5 text-sm font-medium transition-colors border-b-2 -mb-px ${
              activeTab === t ? "border-blue-600 text-foreground" : "border-transparent text-muted-foreground hover:text-foreground"
            }`}>
            {t}
          </button>
        ))}
        ...
        <div className="space-y-8">
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setActiveTab("Upcoming")} className="text-xs text-blue-600 hover:underline">View all</button>
            </div>
            <div className="space-y-3">
              {upcomingInterviews.map(iv => <UpcomingCard key={iv.id} iv={iv} onJoin={() => navigate("/interview")} />)}
            </div>
          </div>
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-base">Practice problems</h2>
              <button onClick={() => setActiveTab("Practice")} className="text-xs text-blue-600 hover:underline">View all</button>
            </div>
            <div className="space-y-2">
              {practiceProblems.slice(0, 3).map(p => <PracticeRow key={p.id} p={p} />)}
            </div>
          </div>
        </div>
      )}
          ...
          <div className="border border-border rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-muted text-muted-foreground text-xs uppercase tracking-wide">
                <tr>{["Role","Company","Date","Score","Result"].map(h => (
                  ...
                  <tr key={iv.id} className="hover:bg-muted/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-20 h-1.5 rounded-full bg-muted overflow-hidden">
                          <div className={`h-full rounded-full ${iv.score >= 75 ? "bg-emerald-500" : "bg-red-400"}`}
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                        iv.result === "Passed" ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"
                      }`}>{iv.result}</span>
                    </td>
                  </tr>
                ))}
      ...
      {activeTab === "Practice" && (
        <div>
          ...
          <div className="border border-border rounded-xl overflow-hidden divide-y divide-border bg-card">
            {practiceProblems.map(p => <PracticeRow key={p.id} p={p} full />)}
          </div>
        </div>
      )}
...
type UpcomingIv = { id: number; role: string; company: string; date: string; time: string; status: string }

function UpcomingCard({ iv, onJoin }: { iv: UpcomingIv; onJoin: () => void }) {
  return (
...
type ProblemItem = { id: number; title: string; topic: string; difficulty: string; solved: boolean }

function PracticeRow({ p, full }: { p: ProblemItem; full?: boolean }) {
  return (
    <div className={`flex items-center justify-between px-4 py-3 ${full ? "" : "border border-border rounded-xl bg-card mb-2"} hover:bg-muted/40 transition-colors`}>
      ...
      <div className="flex items-center gap-2">
        ...
        {!p.solved && <button className="text-xs text-blue-600 hover:underline font-medium">Solve</button>}
      </div>
    </div>
  )
}
...
function ScheduleModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4" onClick={onClose}>
      <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-md shadow-lg" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="text" placeholder="e.g. Frontend Developer" value={role}
              ...
              {["Technical", "HR", "System Design"].map(t => (
                <button key={t} type="button" onClick={() => setType(t)}
                  className={`h-9 rounded-lg border text-xs font-medium transition-colors ${
                    type === t ? "border-blue-600 bg-blue-50 text-blue-700" : "border-border bg-background hover:bg-muted"
                  }`}>
                  {t}
                </button>
              ))}
