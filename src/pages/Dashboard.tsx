          ...
          <div className="border border-border rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-muted text-muted-foreground text-xs uppercase tracking-wide">
                <tr>
                  {["Role", "Company", "Date", "Score", "Result"].map((h) => (
      ...
      {activeTab === "Practice" && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-base">Practice problems</h2>
            <span className="text-xs text-muted-foreground">
              {practiceProblems.filter((p) => p.solved).length}/
              {practiceProblems.length} solved
            </span>
          </div>
...
function PracticeRow({ p, full }: { p: ProblemItem; full?: boolean }) {
  return (
    <div
      className={`flex items-center justify-between px-4 py-3 ${full ? "" : "border border-border rounded-xl bg-card mb-2"} hover:bg-muted/40 transition-colors`}
    >
      <div className="flex items-center gap-3">
        <div
      ...
      <div className="flex items-center gap-2">
        <span
          className={`text-xs font-medium px-2 py-0.5 rounded-full ${difficultyColor[p.difficulty]}`}
        >
          {p.difficulty}
        </span>
        {!p.solved && (
