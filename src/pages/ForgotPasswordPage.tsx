...
export default function ForgotPasswordPage() {
  ...
  function validateReset() {
    ...
    else if (newPassword !== confirmPwd) e.confirmPwd = "Passwords do not match"
    return e
  }
  ...
  async function handleReset(e: React.FormEvent) {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Invalid OTP or request failed"
      setErrors({ otp: msg })
    } finally {
      setLoading(false)
    }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="border border-border rounded-2xl bg-card shadow-sm overflow-hidden">
              ...
              <div key={s} className="flex items-center gap-2">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-semibold transition-colors ${
                  step === s
                    ...
                    : (step === "reset" && s === "email") || step === "success"
                    ? "bg-emerald-500 text-white"
                    : "bg-muted text-muted-foreground"
                }`}>
                  {(step === "reset" && s === "email") || (step === "success" && s !== "success")
                ...
                {i < 2 && <div className={`flex-1 h-0.5 w-8 rounded-full ${
                  (step === "reset" && i === 0) || step === "success" ? "bg-emerald-500" : "bg-muted"
                }`} />}
              </div>
            ))}
          </div>
          ...
          <div className="p-7">
            ...
            {step === "email" && (
              <>
                <h2 className="text-xl font-bold tracking-tight mb-1">Forgot password?</h2>
            ...
            {step === "reset" && (
              <>
                <div className="mb-6">
                  <h2 className="text-xl font-bold tracking-tight mb-1">Reset your password</h2>
